...
function navDropdownItems(kind) {
  if (kind === "services")
    return SERVICES.map((s) => ({
      label: s.title,
      href: `services.html#${s.slug}`,
    }));
  if (kind === "industries")
    return INDUSTRIES.map((i) => ({
      label: i.title,
      href: `industries.html#${i.slug}`,
    }));
  return [];
}
...
function renderFooter() {
  ...
  const marqueeHtml = marqueeRepeated
    .map(
      (item) => `
  ...
  const quickLinks = FOOTER_LINKS.map(
    (l) =>
      `<li><a href="${l.href}" class="text-sm text-creamy/55 transition-colors hover:text-limeglow">${l.label}</a></li>`,
  ).join("");
  const serviceLinks = SERVICES.map(
    (s) =>
      `<li><a href="services.html#${s.slug}" class="text-sm text-creamy/55 transition-colors hover:text-limeglow">${s.title}</a></li>`,
  ).join("");
...
function renderHero() {
  const badges = COMPLIANCE_BADGES.map(
    (b) =>
      `<span class="rounded-full border border-creamy/15 bg-white/[0.03] px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.15em] text-creamy/50">${b}</span>`,
  ).join("");
  return `
  ...
  const cards = [
    {
      ...
      description:
        "Scale a team of one into a team of twenty without rebuilding your hiring pipeline every time.",
    },
    {
      icon: "zap",
      title: "Accelerated Sourcing",
      description:
        "Hire in as little as 5 days, versus the 45+ days typical of domestic recruiting cycles.",
    },
    {
      icon: "shield-check",
      title: "Liability Protection",
      description:
        "We assume compliance, payroll localization, and employer-of-record risk — you get an asset-light workforce.",
    },
  ];
...
function renderPortfolioRows(category) {
  ...
  const rows =
    filtered.length >= 4
      ? [filtered.slice(0, half), filtered.slice(half)]
      : [filtered];
...
function renderPortfolioSection() {
  const filterButtons = PROJECT_CATEGORIES.map(
    (cat) => `
    ...
    .map(() =>
      icon(
        ...
        creamCard
          ...
          : "h-4 w-4 fill-limeglow text-limeglow",
      ),
    )
    .join("");
  return `
...
function renderTestimonials() {
  ...
  const colsHtml = columns
    .map((col, c) => {
      ...
      const cards = loop
        .map((t, i) => testimonialCardHtml(t, (i + c) % 3 === 0))
        .join("");
      return `
...
function renderContactSection(showHeading = true) {
  const inputClass =
    "w-full rounded-2xl border border-creamy/12 bg-white/[0.04] px-5 py-4 text-sm text-creamy placeholder:text-creamy/30 outline-none transition duration-300 focus:border-limeglow/60 focus:bg-white/[0.06] focus:shadow-[0_0_25px_rgba(231,255,158,0.08)]";
  const selectClass = `${inputClass} appearance-none pr-11 cursor-pointer`;
  const serviceOptions = SERVICES.map(
    (s) =>
      `<option value="${s.title}" class="bg-ink-deep text-creamy">${s.title}</option>`,
  ).join("");
  const teamScaleOptions = TEAM_SCALES.map(
    (b) => `<option value="${b}" class="bg-ink-deep text-creamy">${b}</option>`,
  ).join("");
  const badgesHtml = COMPLIANCE_BADGES.map(
    (b) =>
      `<span class="rounded-full border border-creamy/15 bg-white/[0.03] px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.15em] text-creamy/50">${b}</span>`,
  ).join("");
...
function renderServiceDetails() {
  const rows = SERVICES.map((service, i) => {
    ...
    const overviewHtml = service.overview
      .map((p) => `<p class="mt-4 leading-relaxed text-creamy/55">${p}</p>`)
      .join("");
    const itemsHtml = service.items
      .map(
        (item) =>
          `<li class="flex items-center gap-2.5 text-sm text-creamy/70">${icon("check", "h-4 w-4 shrink-0 text-limeglow")}${item}</li>`,
      )
      .join("");
    return `
...
function renderBookCallForm() {
  const inputClass =
    "w-full rounded-2xl border border-creamy/12 bg-white/[0.04] px-5 py-4 text-sm text-creamy placeholder:text-creamy/30 outline-none transition duration-300 focus:border-limeglow/60 focus:bg-white/[0.06] focus:shadow-[0_0_25px_rgba(231,255,158,0.08)] [color-scheme:dark]";
...
function renderBlogContent() {
  const topics = [
    {
      icon: "globe",
      title: "Managing Remote Teams Across Timezones",
      description:
        "Practical frameworks for keeping distributed teams aligned and productive.",
    },
    {
      icon: "shield-check",
      title: "Global Payroll & Compliance 101",
      description:
        "What enterprise buyers need to know about compliant international staffing.",
    },
    {
      icon: "trending-up",
      title: "Building an Operationally Efficient Remote Org",
      description:
        "How fast-growing companies structure remote-first operations for scale.",
    },
    {
      icon: "users",
      title: "A Hiring Manager's Guide to Vetting Remote Talent",
      description:
        "What rigorous background checks and skills testing actually look like.",
    },
    {
      icon: "lock",
      title: "Data Security in Distributed Teams",
      description:
        "Layered security protocols for teams that span borders and time zones.",
    },
    {
      icon: "clock",
      title: "The Real Cost of a 60-Day Time-to-Hire",
      description: "Why slow hiring cycles quietly compound into lost revenue.",
    },
  ];
  const cards = topics
    .map(
